...
export default function BlogSection() {
  ...
  // Only display non-news articles in the standard BlogSection
  const blogPostsOnly = useMemo(() => {
    return BLOG_POSTS.filter((post) => post.category !== "Tin tức");
  }, []);

  // Get unique categories
  const categories = useMemo(() => {
    const cats = blogPostsOnly.map((post) => post.category);
    return ["Tất cả", ...Array.from(new Set(cats))];
  }, [blogPostsOnly]);

  // Filter blog posts based on search term and category
  const filteredPosts = useMemo(() => {
    return blogPostsOnly.filter((post) => {
  ...
  }, [blogPostsOnly, searchTerm, selectedCategory]);
