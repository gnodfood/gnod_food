...
export default function Header({ onOrderNowClick, cartCount, onOpenCart, onOpenAccount }: HeaderProps) {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const sections = ["home", "story", "products", "recipes", "news", "blog", "customer-care", "careers"];
          ...
          <nav className="hidden lg:flex items-center space-x-6 xl:space-x-8">
            {navItems.map((item) => (
              <button
                ...
                className={`font-display text-xs xl:text-sm font-medium tracking-wide transition-colors duration-200 cursor-pointer ${
