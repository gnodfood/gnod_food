...
app.post("/api/chatbot", async (req, res) => {
  try {
    ...
    // To comply with Gemini API, the conversation must start with a "user" turn and strictly alternate roles.
    const contents: any[] = [];
    ...
    // Fallback: If contents is empty (e.g., no user message was yet captured or it started incorrectly),
    // we must supply a default first user message to guarantee API compliance.
    if (contents.length === 0) {
      contents.push({
        role: "user",
        parts: [{ text: "Xin chào" }],
